...
`claude -p` is non-interactive mode. Single turn, because we want a review, not a conversation. Text output, because we're posting it as a comment, not feeding it to a machine. The diff goes in. Markdown comes back. The script posts it to the merge request via the GitLab API. That's the whole trick.

It runs on our existing Mac CI runner, where Claude Code is installed and logged into a Max subscription. The runner was already there for building mobile apps — we just gave it a side gig.

## What the AI Thought of Its Own Code

Here's where things got entertaining.

**Run 1** found the stderr bug from the opening, plus a helpful note that we should probably exclude font files and SVGs from the diff. (We're working on a React Native project. These things show up.) Both valid, both fixed in minutes.
...
Three runs, four real fixes. The tool debugged itself, found a security concern in its own prompt design, and improved its own error reporting. We'll admit to a quiet moment of staring at the screen.
...
Each of these was one push-wait-read-the-log-curse-fix-push cycle. Now they're documented, and the next repository will take five minutes instead of forty.
...
Three reviews in, the answer is yes, with an asterisk shaped like "we'll see how it scales."

The cost is effectively zero — it runs on a Max subscription, no per-review billing. The manual trigger means it only fires when someone actually wants a second opinion. The `allow_failure` flag means even if the script crashes spectacularly, nobody's merge request is held hostage.

It doesn't replace human review. It's more like having a colleague who reads every line of the diff with mechanical patience, checking for the kind of things humans gloss over because they're focused on the bigger picture. Forgot to handle the error case on that API call? The AI noticed. Accidentally left a debug log in the production path? The AI noticed that too. It's relentlessly thorough about the tedious stuff, which leaves the human reviewers free to think about architecture and intent.
...
Three files. Two hours. A CI runner with Claude installed.
